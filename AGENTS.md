...
- Git history currently contains only `first commit`, so no strong pattern exists yet.
...
- Do not commit secrets, local caches, or generated output (`node_modules/`, `dist/`, `.omx/`, `.npm-cache/`).
- Treat `VITE_ENABLE_MOCK_AUTH=true` as local-only development behavior.
